...
- **`schema.py`** holds `SCHEMA_CORE` as a single baseline (embedding DDL folded in), applied idempotently via `CREATE TABLE IF NOT EXISTS` — there is no migration-DML ladder for the base schema. Schema *deltas* on top of that baseline are tracked via `PRAGMA user_version`, applied in a `BEGIN IMMEDIATE … COMMIT` transaction after `SCHEMA_CORE` executes (so a fresh install always has tables before any delta DML runs); a failed delta rolls back and retries on the next connection. The conversations DB is a public contract now; don't evolve it in a way that silently loses a user's synced history. `messages_fts` (a dead FTS index nothing read) has been dropped — `branches_fts`, the live keyword search index, is untouched. The embedding layout is a **two-table design**: `chunks` (one row per exchange — the user turn plus its following assistant turns — carrying the exchange locator, bounded display text, and embedding bookkeeping) and `chunk_vec` (a sqlite-vec virtual table keyed by chunk rowid, 512-dim float vectors). `branches.embedding_version` is retained as a per-branch *watermark* meaning "every current exchange of this branch has a current-version chunk vector"; per-chunk staleness (`chunks.embedding_version`) is the source of truth for query-time filtering. The hot-path framing is unaffected — embedding runs in a detached `sync-current` process, never on the hook thread. Inference memory is bounded by `embeddings.embed_batch`'s planner (`EMBED_BATCH_ATTENTION_BUDGET` = `MODEL_TOKEN_LIMIT²` of attention area — texts × longest² — plus `EMBED_BATCH_MAX_TEXTS` per `model.embed()` call). fastembed's default `batch_size=256` pads each batch to its longest text, and 256×8192-token fp32 activations exceed 20 GB — an observed WSL-killing OOM. The dominant term is **quadratic** in sequence length: the ONNX export's ALiBi attention isn't fused, so the full seq² score matrix materializes (measured single-text peaks: ~1 GB at ~2.2k tokens, ~4 GB at ~4.5k, ~8.4 GB at the 8192 cap). The budget caps every inference call at one max-length text's attention area, so packing can never exceed the single-capped-text worst case — that residual ~8.4 GB peak is accepted for now (lowering the 8192 cap would change vectors and force a full re-embed; revisit if it bites outside backfill). `get_model` passes `enable_cpu_mem_arena=False` because onnxruntime's arena would otherwise pin that peak workspace forever (growing RSS across a backfill; `malloc_trim` can't reclaim arena memory) — with the arena off the transient returns to the OS after every call. New call sites must go through `embed_batch`/`embed_text`, never `model.embed` directly. `messages` carries a `tool_content TEXT` column (populated by `extract_text_content`, alongside `content` for prose) — the existing `tool_summary` and `has_tool_use` columns remain unpopulated dead columns (`branches.tool_counts` already serves that purpose).
...
- **`backfill_embeddings.py` decomposition.** `backfill_embeddings.py` keeps the `run()` orchestrator plus the per-batch `_reclaim_memory` helper (gc + `malloc_trim` between batches, the same RSS discipline as import). `backfill_query.py` owns query construction (`build_selection`, `days_modifier`), the shared constants both the orchestrator and the status reporter agree on (`BATCH_SIZE`, `PID_KEY`, exit codes), and PID cleanup. The CLI command acquires that PID marker before `run()` via `config.try_acquire_pid_file` (skip-not-queue when another instance is alive, exit 0 for scheduler friendliness) — a second concurrent backfill would double the onnxruntime memory peak. `backfill_status.py` owns read-only progress counting (`count_status`), duration formatting, and the `--status` report (`run_status`).
