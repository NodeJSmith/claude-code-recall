...
- **CLI consolidated into a single `ccrecall` entry point** (built on [cyclopts](https://github.com/BrianPugh/cyclopts)). The former skill-facing and helper binaries are now subcommands: `cm-recent-chats` → `ccrecall recent`, `cm-search-conversations` → `ccrecall search`, `cm-session-tail` → `ccrecall tail`, `cm-ingest-token-data` → `ccrecall tokens`, `cm-import-conversations` → `ccrecall import`, `cm-sync-current` → `ccrecall sync-current`, `cm-backfill-summaries` → `ccrecall backfill summaries`, `cm-backfill-embeddings` → `ccrecall backfill embeddings`, `cm-write-config` → `ccrecall write-config`. Behavior, flags, output, and exit codes are unchanged. The five hook entry points (`cm-memory-setup`/`cm-memory-sync`/`cm-memory-context`/`cm-onboarding`/`cm-clear-handoff`) keep their names for now.
