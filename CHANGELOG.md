...
- New runtime dependencies: `sqlite-vec` (vector KNN), `fastembed` (embedding model + ONNX runtime), `numpy` (vector math).
...
- Embedding throttle: inference runs at one thread by default (raise with `cm-backfill-embeddings --threads N`) and the backfill runs at lowered scheduling priority (`nice`), so it never saturates the machine.
- `cm-search-conversations --keyword-only`: skip embedding, use keyword search only.
- `cm-search-conversations --status`: print diagnostic info (vec extension loaded, model name, embedded/total branch count) and exit 0.
- `branch_vec` vec0 virtual table in `conversations.db` storing per-branch 512-dim embeddings.
