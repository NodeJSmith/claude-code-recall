...
def build_context_summary_json(branch_row: dict, messages: list[dict]) -> dict:
    ...
    return {
        ...
        "metadata": {
            ...
            # The count actually split into first_exchanges/last_exchanges above.
            # render_context_summary must key its layout decision off this, not
            # exchange_count (transcript-derived, and routinely higher than
            # len(exchanges) — slash-command and image-only user turns are
            # counted in the transcript but never inserted as message rows).
            "rendered_exchange_count": len(exchanges),
...
def render_context_summary(summary_json: dict) -> str:
    ...
    # Layout must be keyed off the count build_context_summary_json actually
    # split (rendered_exchange_count), not exchange_count — the two routinely
    # disagree (slash-command/image-only user turns inflate exchange_count
    # without a corresponding stored message), which duplicates exchanges
    # between "Where We Left Off" and "Earlier in This Session" or reports a
    # phantom gap. Older stored summaries predate this field and carried no
    # divergence-safe count; fall back to exchange_count, matching the prior
    # (pre-fix) behavior for that stale data until it's regenerated by the
    # SUMMARY_VERSION-gated backfill.
    rendered_exchange_count = meta.get("rendered_exchange_count")
    if rendered_exchange_count is None:
        rendered_exchange_count = exchange_count

    if rendered_exchange_count <= SHORT_SESSION_MAX_EXCHANGES:
        ...
        gap = rendered_exchange_count - len(first_exs) - len(last_exs)
        if gap > 0:
