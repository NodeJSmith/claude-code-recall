...
def build_output(conn: sqlite3.Connection) -> dict:
    ...
    if total_ephem_5m > 0 or total_ephem_1h > 0:
        ...
        dominant_cache_tier = (
            "5m"  # default — Claude Code switched to ephemeral_5m ~Apr 3 2026
        )
    ...
    insights = _build_insights(
        ...
        edit_retries=total_edit_retries,
...
def _build_insights(**kw) -> list[dict]:
    ...
    if base_pct > 30 and avg_base > 0:
        ...
        waste_tok = (
            reducible_base * sessions
        )  # excess base * sessions (conservative: 1 turn each)
        waste_dollars = _waste_usd(waste_tok)
        insights.append(
            {
