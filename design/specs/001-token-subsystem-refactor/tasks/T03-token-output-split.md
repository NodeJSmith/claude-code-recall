...
5. **`build_kpis(cur)`** for the top KPI block, `date_range`, the `kpis` dict, and the intermediate totals (`total_output`, `total_input`, `total_cache_cliffs`, `total_max_token_stops`, `total_bash_antipatterns`, `total_redundant_reads`, `total_edit_retries`, `total_thinking`, `total_tool_errors`, `global_cache_ratio`, `dominant_cache_tier`, `total_cost_usd`, `total_tool_calls`) that feed the `build_insights_and_trends` call and the final `kpis` assembly. Decide the cleanest return shape (a small dataclass or a dict of intermediates) — these values cross from KPI computation into both the insight call and the final dict.
6. **`response_time_dist`** is computed before the insight call and its value is reused (passed into `build_insights_and_trends`), not recomputed — preserve that single computation.
7. **Cost loops** (`model_split`, `cost_by_day`, `cost_by_project`) call the T02 helper instead of inline `get_pricing`+`turn_cost`. Remember `model_split` passes `token_indices=[1,2,4,5,6,7]` (skips `thinking` at index 3); the other two pass their contiguous indices. **Remove the inline loops** — do not leave them beside the helper call (Replacement Target).
8. **Detail+total SQL pairs** (`bash_antipatterns`/`total_bash_antipatterns`, `redundant_reads`/`total_redundant_reads`, `edit_retries`/`total_edit_retries`): express each shared predicate/join once. Where the total is reconstructable from the detail rows, derive it; where it needs a different aggregation (no LIMIT), hoist the shared join/predicate text to a module-level SQL fragment constant used by both queries so the text isn't duplicated. `_BASH_ANTIPATTERN_PREDICATE` is already a shared constant — keep using it.
