...
- `_extract_usage`, `_detect_cache_ttl_ms`, `compute_session_analytics`, `_normalize_worktree_path`, `project_slug`, `discover_jsonl_files`, `should_skip_file`, `record_import` are all outside `parse_session` and **out of scope** — do not touch them.
