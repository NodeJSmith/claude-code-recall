...
- **Multi-insight case — exercise the formatting paths:** pass **non-empty** list args for `bash_antipattern_projects`, `top_redundant_files`, and `edit_retry_projects` (each a list of the small dicts those builders expect, e.g. `[{"project": "p", "antipatterns": 3, ...}]`). If left at their empty-list defaults, the golden encodes only the fallback strings ("unknown"/"various files") and never pins the `proj_detail`/`file_detail`/`cmd_detail` formatting — which is exactly the branch the T04 split must preserve. Pass `cost_by_project` non-empty too (with a `total_cost_usd` making the top project >40%) so the Cost Concentration insight and its formatting are pinned.
