...
5. **`tests/test_db.py` — add the tests that now pass:**
   ...
   - **AC#4:** build a DB that looks like an existing v6 DB — apply `SCHEMA`, insert sample rows into `projects`/`sessions`/`branches`/`messages`, then **manually** `CREATE TABLE token_snapshots (...)` and insert a row (this simulates the inert legacy table left by the old `migrate_columns`; `SCHEMA` deliberately no longer includes `token_snapshots`, so the test must create it itself — that's the point of the test). Set `PRAGMA user_version = 6`, close. Then open it via `get_db_connection` and assert it returns successfully and the sample rows in `projects`/`sessions`/`branches`/`messages`/`token_snapshots` are intact (counts unchanged, no table dropped). (`projects`/`sessions` rows are needed anyway for the FK chain: `sessions`→`projects`, `branches`→`sessions`, `messages`→`sessions`.) This proves `get_db_connection`'s `CREATE TABLE IF NOT EXISTS` apply never touches the pre-existing legacy `token_snapshots`.

Run the full suite (`uv run pytest -q`) — it must pass. Confirm `grep -rn "ccrecall.migrations\|migrate_columns\|migrate_db\|user_version" src/ tests/` returns nothing (AC#5, and confirms FR#5's `user_version` retirement across both trees).

See `## Architecture` steps 2-5, `## Migration`, and `## Edge Cases` (existing-v6-DB open) in the design doc.

## Focus
- `get_db_connection` is `src/ccrecall/db.py:241`. The schema-apply block it already contains (lines 263-271, inside `if not migrated:`) is the body to keep — hoist it out of the `if not migrated:` guard so it runs unconditionally, and delete the `migrate_db`/reconnect scaffolding around it. **Separately**, the `migrate_columns(conn)` call at line ~274 sits *outside* that block (it currently runs on every path) — deleting the `migrate_db` scaffolding does NOT remove it; you must delete line ~274 explicitly too (per Prompt step 1). The end state of `get_db_connection`: open conn → `apply_base_pragmas` → detect FTS → `executescript(SCHEMA_CORE)` + FTS variant → `commit` → the unchanged `load_vec` block. `SCHEMA_CORE`, `SCHEMA_FTS5`, `SCHEMA_FTS4`, `detect_fts_support` are already imported in `db.py`.
...
- [ ] FR#5: No `PRAGMA user_version` read or write remains in `src/` or `tests/` (`grep -rn "user_version" src/ tests/` clean).
