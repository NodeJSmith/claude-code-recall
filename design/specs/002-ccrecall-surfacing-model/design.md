...
**Tier 2 — Reactive caveat (small).** The recall/search path, after assembling results, computes embedding availability at query time via `chunk_vec_queryable(conn)` (the table-probe the search path already uses for "is vec usable?" — NOT `vec_available`, which re-loads the extension with side effects) plus `branch_embedding_coverage(conn)`, and appends one caveat line when embeddings are unavailable or coverage is below a named threshold constant (default 0.95 — defined alongside the other surfacing constants in `health.py`). No new persisted state; independent of tier 3. Note the search path already prints `"search: vector index unavailable, using keyword search"` to stderr (`search_conversations.py:587`) — the results caveat consolidates with / supersedes that signal rather than duplicating it.
