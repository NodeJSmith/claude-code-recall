...
- The recall path already loads vec for search; reuse `vec_available(conn)` and `db.branch_embedding_coverage(conn)` (returns `(embedded, total)`) from `src/ccrecall/db.py` — do not re-probe capability separately.
- Append the caveat when: `vec_available` is False (semantic search degraded to keyword), OR `embedded / total < RECALL_CAVEAT_COVERAGE_THRESHOLD` (the `0.95` constant defined in `health.py`, T01). Guard against `total == 0`.
