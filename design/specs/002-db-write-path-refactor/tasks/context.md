...
## Key Decisions
1. **Behavior-preserving only.** For identical inputs and starting DB state, the resulting schema, `user_version`, every row, and all return values are identical to current behavior — with exactly **one** in-scope delta: `migrate_db`'s reconnected connection now has `foreign_keys=ON` (connection-scoped, on a connection closed immediately after DDL — consistency-hardening, not a behavioral change to the created schema).
...
- **No smuggled behavior changes.** If a split surfaces a latent bug, note + file separately; preserve current behavior including warts. The pins encode current behavior. The *only* deliberate delta is `foreign_keys=ON` (FR#3/AC#3).
