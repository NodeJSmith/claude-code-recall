...
- `sync_branch(conn, cursor, branch, messages, uuid_to_msg_id, existing_branches, session_id, vec_writable)` — the per-branch loop body (lines 210-366). It further delegates:
  - `upsert_branch(cursor, branch, branch_meta, exchange_count, files_json, commits_json, tool_counts_json, session_id, existing_branches) -> int` — the INSERT-vs-UPDATE + the single-active-branch enforcement → `branch_db_id`.
