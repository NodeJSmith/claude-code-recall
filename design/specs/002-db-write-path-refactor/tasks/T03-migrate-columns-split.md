...
**2. Uniform version dispatch.** Extract the inline v1, v2, v4 bodies into `migrate_v1(conn, cursor)`, `migrate_v2(conn, cursor)`, `migrate_v4(conn, cursor, db_path)` to match the already-delegated `_backfill_origin`(v3)/`_migrate_v5`/`_migrate_v6`. **Move the `PRAGMA user_version = N` + `conn.commit()` INTO each `migrate_vN` helper** (the v5/v6 convention) so every gate is self-contained — including v1/v2/v4 whose bumps currently live inline in `migrate_columns` (lines 229-259). Each bump still runs exactly once, after its body, gated on the same `version < N` condition — behavior-identical.
- **v3 (`_backfill_origin`) — handle with a thin wrapper, do not touch `_backfill_origin`.** `_backfill_origin` already ends with its own `conn.commit()` (line 361), and `db.py`/tests reference it by name, so leave its body and name untouched. Add a thin `migrate_v3(conn, cursor)` that calls `_backfill_origin(conn, cursor)` then runs `PRAGMA user_version = 3` + `conn.commit()`. This reproduces today's exact sequence (`_backfill_origin` commits its row updates, then `migrate_columns` bumps+commits — two commits) byte-for-byte; do NOT insert the PRAGMA inside `_backfill_origin` (that would collapse to one commit — a subtle change). The dispatch then calls `migrate_v3(...)` uniformly like every other gate.
...
- [ ] FR#6: each version gate v1–v6 is reachable through a named helper invoked uniformly from `migrate_columns`; v1/v2/v4 are extracted helpers (and v3 via the `migrate_v3` wrapper) like v5/v6; **no `PRAGMA user_version` and no bare `conn.commit()` remain directly in `migrate_columns`** — every commit moved into the DDL/version helper that owns its operation (commit cadence preserved).
