...
- **FR#3** `migrate_db(conn)` returns the same boolean and performs the same nuke-and-recreate / no-op decision as before for each starting schema (pre-v3, fresh, already-v3), and its reconnect path now additionally sets `foreign_keys=ON`.
...
- **Minimal test DBs missing tables.** `_backfill_origin` and `_migrate_project_paths` guard on `sessions`/`projects`/`import_log` existence and return early. The decomposition must preserve those guards — a split that moves the guard away from its early-return changes behavior on minimal DBs.
...
- **AC#3** A `migrate_db` test pins the boolean return and DB effect for each starting schema (pre-v3 → True + recreate, fresh → False, already-v3 → False) — `test_db.py:TestMigrateDb` already covers the boolean/decision matrix. **Add** an assertion that the reconnected DB has `foreign_keys=ON` (the one in-scope behavior delta). (FR#3)
...
- **No smuggled behavior changes.** If the split surfaces a latent migration or sync bug, do not fix it here — note it, file it separately, preserve current behavior including warts. The pins encode current behavior. The single deliberate behavior delta in this work is the `foreign_keys=ON` addition to `migrate_db`'s reconnect (FR#3) — it is explicitly in scope and pinned by AC#3; nothing else changes.
...
- **Pragma relocation.** Relocate `apply_base_pragmas` from `db.py` to `models.py`. `db.py` imports it from `models` (its two internal call sites, db.py:254 and db.py:261, unchanged in behavior). Note on the fix: `journal_mode=WAL` persists to the file while `foreign_keys`/`busy_timeout` are connection-scoped and the recreate connection is closed immediately after DDL — so adding `foreign_keys=ON` is consistency-hardening, not a behavioral change to the created schema. AC#3 pins it regardless.
...
- **Add the `foreign_keys=ON` assertion** to the `migrate_db` reconnect case in `TestMigrateDb` — the one new behavior (FR#3/AC#3).
...
- For identical starting DB state and inputs, the resulting schema, `user_version`, and every row written by `migrate_columns`/`migrate_db`/`sync_session` are identical to current behavior — except the single in-scope delta: `migrate_db`'s reconnected connection now has `foreign_keys=ON` (connection-scoped, on a connection closed immediately after DDL).
