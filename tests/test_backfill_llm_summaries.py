...
class TestBackfillLlmSummaries:
    ...
    def test_pid_guard_skips_live_worker_reaps_stale_marker_and_cleans_up_on_success_and_failure(
        ...
        from ccrecall.hooks import backfill_llm_summaries as worker_reloaded

        monkeypatch.setattr(worker_reloaded, "load_settings", lambda: {"db_path": str(tmp_path / "unused.db")})
        monkeypatch.setattr(
            worker_reloaded, "setup_logging", lambda *_args, **_kwargs: logging.getLogger("test-worker")
        )
        monkeypatch.setattr(worker_reloaded, "_run", lambda **_kwargs: worker_reloaded.EXIT_OK)

        assert worker_reloaded.run() == worker_reloaded.EXIT_OK
        assert not stale_marker.exists()

        cleanup_calls: list[str] = []
        monkeypatch.setattr(worker_reloaded, "remove_pid_file", lambda key: cleanup_calls.append(key))
        monkeypatch.setattr(worker_reloaded, "try_acquire_pid_file", lambda _key: True)
        monkeypatch.setattr(worker_reloaded, "_run", lambda **_kwargs: (_ for _ in ()).throw(RuntimeError("boom")))

        with pytest.raises(RuntimeError, match="boom"):
            worker_reloaded.run()

        assert cleanup_calls == [worker_reloaded.PID_KEY]
