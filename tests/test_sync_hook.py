...
class TestSyncCurrentTranscriptDiscovery:
    def test_stop_sync_source_never_imports_or_invokes_recap_provider(self):
        source = Path(sync_current.__file__).read_text(encoding="utf-8")
        assert "llm_summarizer" not in source
        assert "invoke_claude" not in source
        assert "_spawn_llm_summary_worker" not in source
