...
#: Directory components that are never first-party source: virtualenvs, caches, and build
#: output. Without this filter, rglob over a scan dir can pull in third-party site-packages
#: (e.g. a local ``.venv``) and report them as house-style violations.
EXCLUDED_PARTS = frozenset({".venv", "site-packages", "__pycache__", ".git", "node_modules"})
